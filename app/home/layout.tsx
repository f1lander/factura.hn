...
const mobileNavItems = [
  ...
  { title: "Crear", href: "/home/invoices/create-invoice", icon: () => <span></span> }, // Empty icon component
          ...
          if (index === 2) {
            ...
            if (isCreateInvoicePage) {
              return (
                ...
                >
                  <div className={`absolute -top-6 flex items-center justify-center w-14 h-14 rounded-full shadow-lg ${isButtonDisabled || isSubmitting ? 'bg-gray-400' : 'bg-green-600'} text-white`}>
                    {isSubmitting ? (
...
function MainContent({ children, pageTitle, pageIcon: PageIcon }: { 
  ...
  return (
    <>
      ...
      <SidebarInset className="flex w-full flex-col overflow-hidden">
        ...
        <div className={`flex-1 w-full overflow-y-auto ${isMobile ? 'pb-24 p-1' : ''}`}>
          {children}
        </div>
