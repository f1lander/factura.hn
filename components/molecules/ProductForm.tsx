...
export const ProductForm: React.FC<ProductFormProps> = ({ product, onSubmit, onCancel }) => {
  const { register, handleSubmit, watch, control, formState: { errors } } = useForm<Product>({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{product ? 'Editar Producto/Servicio' : 'Crear Nuevo Producto/Servicio'}</CardTitle>
        <CardDescription>
          {product
        ...
        <form onSubmit={handleSubmit(onSubmitForm)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={isService}
            />
            {errors.quantity_in_stock && <p className="text-red-500 text-sm">{errors.quantity_in_stock.message}</p>}
          </div>
        </form>
      </CardContent>
