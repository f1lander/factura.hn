...
const InvoiceDashboardCharts = ({ invoices }: { invoices: Invoice[] }) => {
  ...
  const getMonthlyRevenue = () => {
    ...
    invoices.forEach((invoice) => {
      const month = new Date(invoice.date).toLocaleString('es-ES', {
        month: 'short',
      });
  ...
  const getTopCustomers = () => {
    ...
    invoices.forEach((invoice) => {
      customerRevenue[invoice.customers.name] =
        (customerRevenue[invoice.customers.name] || 0) + invoice.total;
    });
    ...
    <div className='flex flex-wrap gap-4'>
              ...
              <div className='bg-gray-100 rounded-lg p-3 flex flex-col justify-between'>
                ...
                <div>
                  <div className='text-xl font-bold'>
                    L.{' '}
                    {getTotalsByStatus().total.amount.toLocaleString('es-HN', {
              ...
              <div className='bg-green-50 rounded-lg p-3 flex flex-col justify-between'>
                ...
                <div>
                  <div className='text-xl font-bold text-green-700'>
                    L.{' '}
                    {getTotalsByStatus().paid.amount.toLocaleString('es-HN', {
                      minimumFractionDigits: 2,
                    })}
                  </div>
              ...
              <div className='bg-yellow-50 rounded-lg p-3 flex flex-col justify-between'>
                ...
                <div>
                  <div className='text-xl font-bold text-yellow-700'>
                    L.{' '}
                    {getTotalsByStatus().pending.amount.toLocaleString(
                      'es-HN',
                      { minimumFractionDigits: 2 }
                    )}
                  </div>
              ...
              <div className='bg-red-50 rounded-lg p-3 flex flex-col justify-between'>
                ...
                <div>
                  <div className='text-xl font-bold text-red-700'>
                    L.{' '}
                    {getTotalsByStatus().cancelled.amount.toLocaleString(
                      'es-HN',
                      { minimumFractionDigits: 2 }
                    )}
                  </div>
      ...
      <Card className='flex-grow basis-full sm:basis-[calc(50%-0.5rem)] lg:basis-[calc(25%-0.75rem)]'>
        <CardHeader>
          ...
          <CardDescription>
            Clientes con mayor valor total de facturas
          </CardDescription>
        </CardHeader>
        <CardContent className='h-[200px]'>
      ...
      <div className='grid grid-cols-1 w-full'>
        <Card className='flex-grow basis-full sm:basis-[calc(50%-0.5rem)] lg:basis-[calc(25%-0.75rem)]'>
          <CardHeader>
            ...
            <CardDescription>
              Tendencia de ingresos en los últimos meses
            </CardDescription>
          </CardHeader>
          <CardContent className='h-[200px]'>
